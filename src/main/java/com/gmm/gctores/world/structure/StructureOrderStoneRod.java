...
public class StructureOrderStoneRod extends ElementsGctOresMod.ModElement
{
    public StructureOrderStoneRod(final ElementsGctOresMod instance) {
        super(instance, 488);
    }
        ...
        if (random.nextInt(1000000) + 1 <= 100000) {
            for (int count = random.nextInt(1) + 1, a = 0; a < count; ++a) {
                ...
                if (biomeCriteria) {
                    ...
                    final Template template = ((WorldServer)world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation("gct_ores", "order_rod_1"));
                    if (template == null) {
                        return;
                    }
