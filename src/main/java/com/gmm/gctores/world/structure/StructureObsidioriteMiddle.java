...
public class StructureObsidioriteMiddle extends ElementsGctOresMod.ModElement
{
    public StructureObsidioriteMiddle(final ElementsGctOresMod instance) {
        super(instance, 499);
    }
    ...
    public void generateWorld(final Random random, final int i2, final int k2, final World world, final int dimID, final IChunkGenerator cg, final IChunkProvider cp) {
        ...
        if (random.nextInt(1000000) + 1 <= 1000) {
            for (int count = random.nextInt(1) + 1, a = 0; a < count; ++a) {
                ...
                final Template template = ((WorldServer)world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation("gct_ores", "obisidiorite_middle"));
                if (template == null) {
                    return;
                }
