...
public class StructureOrder1 extends ElementsGctOresMod.ModElement
{
    public StructureOrder1(final ElementsGctOresMod instance) {
        super(instance, 405);
    }
    ...
    public void generateWorld(final Random random, final int i2, final int k2, final World world, final int dimID, final IChunkGenerator cg, final IChunkProvider cp) {
        ...
        if (random.nextInt(1000000) + 1 <= 10000) {
            for (int count = random.nextInt(1) + 1, a = 0; a < count; ++a) {
                ...
                final Template template = ((WorldServer)world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation("gct_ores", "order_s1"));
                if (template == null) {
                    return;
                }
