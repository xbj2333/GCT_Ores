...
public class StructureEndWitheriumComet extends ElementsGctOresMod.ModElement
{
    public StructureEndWitheriumComet(final ElementsGctOresMod instance) {
        super(instance, 392);
    }
    ...
    public void generateWorld(final Random random, final int i2, final int k2, final World world, final int dimID, final IChunkGenerator cg, final IChunkProvider cp) {
        ...
        if (random.nextInt(1000000) + 1 <= 1000) {
            for (int count = random.nextInt(1) + 1, a = 0; a < count; ++a) {
                ...
                final IBlockState require = Blocks.AIR.getDefaultState();
                ...
                if (blockCriteria) {
                    ...
                    final Template template = ((WorldServer)world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation("gct_ores", "end_witherium_1"));
                    if (template == null) {
                        return;
                    }
