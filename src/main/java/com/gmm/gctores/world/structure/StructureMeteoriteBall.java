...
public class StructureMeteoriteBall extends ElementsGctOresMod.ModElement
{
    public StructureMeteoriteBall(final ElementsGctOresMod instance) {
        super(instance, 496);
    }
    ...
    public void generateWorld(final Random random, final int i2, final int k2, final World world, final int dimID, final IChunkGenerator cg, final IChunkProvider cp) {
        ...
        if (random.nextInt(1000000) + 1 <= 10000) {
            for (int count = random.nextInt(1) + 1, a = 0; a < count; ++a) {
                ...
                final int m = Math.abs(random.nextInt(Math.max(1, height)) - 24);
                if (world.isRemote) {
                    return;
                }
                final Template template = ((WorldServer)world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation("gct_ores", "meteorite_ball"));
                if (template == null) {
                    return;
                }
