...
public class StructureOrderStoneRod2 extends ElementsGctOresMod.ModElement
{
    public StructureOrderStoneRod2(final ElementsGctOresMod instance) {
        super(instance, 489);
    }
    ...
    public void generateWorld(final Random random, final int i2, final int k2, final World world, final int dimID, final IChunkGenerator cg, final IChunkProvider cp) {
        ...
        if (random.nextInt(1000000) + 1 <= 50000) {
            for (int count = random.nextInt(1) + 1, a = 0; a < count; ++a) {
                ...
                if (((ResourceLocation)Biome.REGISTRY.getNameForObject(biome)).equals(new ResourceLocation("gct_ores:order_basin"))) {
                    biomeCriteria = true;
                }
                if (((ResourceLocation)Biome.REGISTRY.getNameForObject(biome)).equals(new ResourceLocation("gct_ores:order_plain"))) {
                    biomeCriteria = true;
                }
                if (biomeCriteria) {
                    if (world.isRemote) {
                    ...
                    final Template template = ((WorldServer)world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation("gct_ores", "order_rod_2"));
                    if (template == null) {
                        return;
                    }
