...
import com.gmm.gctores.procedure.ProcedureWrenchBreak;
...
public class ItemSenterianWrench extends ElementsGctOresMod.ModElement
{
    @GameRegistry.ObjectHolder("gct_ores:senterian_wrench")
    public static final Item block;
    
    public ItemSenterianWrench(final ElementsGctOresMod instance) {
        super(instance, 461);
    }
    ...
    public void initElements() {
        this.elements.items.add(() -> ((Item)new ItemToolCustom() {
            public EnumActionResult onItemUse(final EntityPlayer entity, final World world, final BlockPos pos, final EnumHand hand, final EnumFacing direction, final float hitX, final float hitY, final float hitZ) {
                ...
                ProcedureWrenchBreak.executeProcedure($_dependencies);
                return retval;
            }
        }.setTranslationKey("senterian_wrench").setRegistryName("senterian_wrench")).setCreativeTab(TabCTab.tab));
    }
    
    @SideOnly(Side.CLIENT)
    @Override
    public void registerModels(final ModelRegistryEvent event) {
        ModelLoader.setCustomModelResourceLocation(ItemSenterianWrench.block, 0, new ModelResourceLocation("gct_ores:senterian_wrench", "inventory"));
    }
    ...
    private static class ItemToolCustom extends Item
    {
        ...
        public Multimap<String, AttributeModifier> getItemAttributeModifiers(final EntityEquipmentSlot equipmentSlot) {
            ...
            if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
                multimap.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(), new AttributeModifier(ItemToolCustom.ATTACK_DAMAGE_MODIFIER, "Tool modifier", -3.0, 0));
